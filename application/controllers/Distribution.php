...
class Distribution extends MY_Controller {
	...
	public function update()
	{
		...
		{	
		$user_updata += array('password'=>md5($data_save['password']));
		}
		$user_update_record = $this->Distribution_model->update('users',$user_updata,array('id'=>$data_save['distribution_id']));
		$id = $data_save['id'];
		...
		unset($data_save['id']);
		$update_distributor = $this->Distribution_model->update('distribution',$data_save,array('id'=>$id));
		redirect('distribution');		
	}
	...
	public function csv_upload()
	{
		...
		{	
			if(!empty($_FILES))
			{
				...
				$scm_code_ckeck = $this->Distribution_model->view_scm_code_ckeck($column,$data_value,$table_data);
